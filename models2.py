		...
		with tf.Session() as session:
			...
			with open('results.csv', 'w', newline='') as csvfile:
				...
				#Loading image
					...
					fileName = os.path.join(ROOT_PATH, str(i) + '_black_white_black_white.jpg')
					...
					run_dict = {self.x : image}
	
					predictionArray = session.run(self.d_out_real, feed_dict = run_dict)
					...
					#print(i)
					#print(indexMax)
					print(fileName, classes[indexMax])
					writer.writerow(classes[indexMax-1])
